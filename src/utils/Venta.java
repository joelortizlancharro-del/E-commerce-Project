...
public class Venta {
    ...
    String sqlTiquet = "INSERT INTO tiquets (data_compra, dni_client, total_base, total_iva, total_final) VALUES (CURDATE(), ?, 0, 0, 0)";
    ...
    try (PreparedStatement psTiquet = conn.prepareStatement(sqlTiquet, Statement.RETURN_GENERATED_KEYS);
        ...
        for (int i = 0; i < idArticles.size(); i++) {
            ...
            psLinia.setInt(3, quantitats.get(i));

            psLinia.executeUpdate();
            System.out.println("Article " + idArticles.get(i) + " insertat al tiquet " + idTiquet);

            psStock.setInt(1, quantitats.get(i));
            psStock.setInt(2, idArticles.get(i));

            psStock.executeUpdate();

            System.out.println("Stock actualitzat per article " +
                    idArticles.get(i));
        }
